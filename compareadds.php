...
?>
        ...
        <script type="text/javascript">
            ...
            jQuery(function() { // begin the document ready function
                ...
                // start over ... window.location no work ie7
            ...
            function setPin(aLoc) {
                try {
                    ...
                    function(shapeLayer, findResults, places, moreResults, errorMsg) {
                        ...
                        if ((aLoc++ < (locArr.length - 1)) && (locArr[aLoc][0] != '')) {
                            setPin(aLoc);
                        }
                        else {
...
print "<div class='info-msg'><a style='text-decoration: none;' href='" . $homeframe . "' target='_parent'><span id='home_button' alt='Start Over' title='Start Over' style='cursor: pointer;'><img src='images/ca_icon_startover.gif' style='vertical-align: middle; border: 0;'/> Start Over</span></a>&nbsp;&nbsp;&nbsp;<span id='print_button' alt='Print' title='Print' style='cursor: pointer;'><img src='images/ca_icon_print.gif' style='vertical-align: middle;'/> Print</span></div>\n";
...
function make_pct($num)
{
    ...
    return $updown . number_format(abs($num), 2, ".", ",") . "%";
}

print "<td>Up/Down:</td>\n";
if ((trim($row1['RTOTAPR']) > 0) && (trim($row1['2009TOTAPR']) > 0))  {print "<td>" . make_pct((trim($row1['2009TOTAPR'])-trim($row1['RTOTAPR']))/trim($row1['RTOTAPR'])*100) . "</td>\n";}ELSE{print"<td>NA</td>";}
if ($qrows2 >= 1) {if ((trim($row2a['RTOTAPR']) > 0) && (trim($row2a['2009TOTAPR']) > 0))  {print "<td>" . make_pct((trim($row2a['2009TOTAPR'])-trim($row2a['RTOTAPR']))/trim($row2a['RTOTAPR'])*100) . "</td>\n";}ELSE{print"<td>NA</td>";}}
if ($qrows2 >= 2) {if ((trim($row2b['RTOTAPR']) > 0) && (trim($row2b['2009TOTAPR']) > 0))  {print "<td>" . make_pct((trim($row2b['2009TOTAPR'])-trim($row2b['RTOTAPR']))/trim($row2b['RTOTAPR'])*100) . "</td>\n";}ELSE{print"<td>NA</td>";}}
if ($qrows2 >= 3) {if ((trim($row2c['RTOTAPR']) > 0) && (trim($row2c['2009TOTAPR']) > 0))  {print "<td>" . make_pct((trim($row2c['2009TOTAPR'])-trim($row2c['RTOTAPR']))/trim($row2c['RTOTAPR'])*100) . "</td>\n";}ELSE{print"<td>NA</td>";}}
if ($qrows2 >= 4) {if ((trim($row2d['RTOTAPR']) > 0) && (trim($row2d['2009TOTAPR']) > 0))  {print "<td>" . make_pct((trim($row2d['2009TOTAPR'])-trim($row2d['RTOTAPR']))/trim($row2d['RTOTAPR'])*100) . "</td>\n";}ELSE{print"<td>NA</td>";}}
